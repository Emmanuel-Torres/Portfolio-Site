...
const getStatus = () => {
  return execSync("systemctl status wg-quick@wg0.service", { uid: 1000 });
};
...
const getVmPublicKey = () => {
  return execSync("sudo cat /etc/wireguard/publickey", { uid: 1000 });
};
...
const getPeers = () => {
  ...
  return execSync(cmd, { uid: 1000 }).toString().split("\n");
};
