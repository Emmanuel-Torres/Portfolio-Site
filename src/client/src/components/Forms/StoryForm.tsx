...
const StoryForm: FC<Props> = (props): JSX.Element => {
    const [title, setTitle] = useState<string>(props.story?.title ?? '');
    const [content, setContent] = useState<string>(props.story?.content ?? '');
    const [mainImageUrl, setMainImageUrl] = useState<string>(props.story?.mainImageUrl ?? '');
    const [tags, setTags] = useState<string>(props.story?.tags ?? '');
